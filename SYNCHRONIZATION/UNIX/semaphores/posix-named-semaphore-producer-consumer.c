...
int main(int argc, char** argv){
	...
	for (i=0; i<SIZE; i++){
		sprintf(buff,"slot%d\0",i); // ad ogni ciclo scrivo su buff slot+i
		sem_free_slot[i] = sem_open(buff,O_CREAT,0666,1);  // inizializzo i semafori named con nome contenuto in buff
		if (sem_free_slot[i] == NULL){
			printf("cannot create free slot semaphore at iteration %d\n",i);
			exit(0);
		}	
		sem_unlink(buff); //rimuovo il nome dal VFS, ho ancora il puntatore da usare per l'accesso ai semafori
	...
	if (cons == 0){ // il secondo child esegue il consumer
		consumer();
	}
