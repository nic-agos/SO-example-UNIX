...
retry:
	if(lock(&global_lock)){  // cerca di prendere il controllo bloccando la serratura, se non fallisce vuol dire che il controllo è stato preso da me
		...
		global_lock = 0;  // rilascio la serratura
	}
...
retry:
	if(lock(&global_lock)){  // cerco di prendere il lock
		if(counter > 0){
			value = v[my_index];
